...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default=os.path.join('..', '..', 'animal-dataset', 'data', 'clean'))
    ...
    datagen_train = animal_generator(path=path_train, output_type=args.output_type,
                                     include_landmarks=args.include_landmarks, batch_size=args.batch_size,
                                     shuffle=True, flip_horizontal=args.flip_horizontal,
                                     rotate=args.rotate, rotate_90=args.rotate_90,
                                     rotate_n=args.rotate_n, crop=args.crop,
                                     crop_scale_balanced_black=args.crop_scale_balanced_black, crop_scale_balanced=args.crop_scale_balanced,
    ...
    test_validation_args = dict(output_type=args.output_type,
                                ...
                                flip_horizontal=False, rotate=False,
                                rotate_90=False, rotate_n=0,
                                crop=False, crop_scale_balanced_black=False,
                                crop_scale_balanced=False, sampling_method_resize=Image.LANCZOS)
    datagen_val = animal_generator(path=path_val, **test_validation_args)
    datagen_test = animal_generator(path=path_test, **test_validation_args)
    pretrained_net = mobilenet_v2.MobileNetV2(input_shape=pet.utils.general.image_shape, include_top=False, pooling=args.pooling)
    ...
    if args.loss_fn in ('iou', 'iou_and_mse_landmarks') and args.output_type == 'bbox':
        model.compile(optimizer=keras.optimizers.Adam(), loss='mse', metrics=[pet.utils.general.iou])
        model.fit_generator(generator=datagen_train, epochs=1, shuffle=True, steps_per_epoch=50, workers=3)
    if args.output_type == 'bbox':
        metrics = [pet.utils.general.iou, 'mse']
        monitor, mode = 'val_iou', 'max'
    else:
        metrics = []
        monitor, mode = 'val_loss', 'min'
    loss_fn = pet.utils.general.get_loss_fn(args.output_type, args.loss_fn, args.iou_and_mse_landmarks_ratio)
    ...
    train_history = model.fit_generator(generator=datagen_train, epochs=args.epochs, shuffle=True,
                                        ...
                                        callbacks=[TensorBoard(log_dir=os.path.join('logs', exp_name)),ReduceLROnPlateau(factor=args.ReduceLROnPlateau_factor, 
                                            patience=args.ReduceLROnPlateau_patience, verbose=1, monitor=monitor, mode=mode),
                                            EarlyStopping(patience=(2 * args.ReduceLROnPlateau_patience) + 3, verbose=1, monitor=monitor, mode=mode),
                                            ModelCheckpoint(model_path, verbose=1, save_best_only=True, monitor=monitor, mode=mode)])
    print('Testing...')
    custom_objects = pet.utils.general.get_custom_objects(args.loss_fn, loss_fn)
    ...
    print(test_metrics)
    pet.utils.general.append_hp_result(path=args.hpsearch_file, exp_name=exp_name, args=vars(args), history=train_history.history, test_metrics=test_metrics, monitor=monitor, mode=mode)
